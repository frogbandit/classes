$(document).on('submit', '#find', function(evt) {
	...
	$.post('/', {course: course}).done(function(response) {
		...
		for (i=0; i < courses.length; i++){
			course1 = JSON.parse(JSON.stringify(courses[i][0]['course']));
			grade1 = JSON.parse(JSON.stringify(courses[i][0]['grade']));
			course2 = JSON.parse(JSON.stringify(courses[i][1]['course']));
			grade2 = JSON.parse(JSON.stringify(courses[i][1]['grade'])); 
			action = JSON.parse(JSON.stringify(actions[i]));
